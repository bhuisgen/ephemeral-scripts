...
function_exists hook_init && hook_init ||
exit_error "Function hook_init() failed"
...
function_exists hook_check && hook_check ||
exit_error "Function hook_check() failed"
...
exit_error "Function hook_after_create_snapshot() failed"

echo "Mounting snapshot ..."
mkdir -p "$MOUNT_PATH"
mount "$LV_SNAPSHOT_DISK" "$MOUNT_PATH" ||
exit_error "Failed to mount snapshot"

echo "Starting backup ..."
function_exists hook_backup &&
hook_backup "$MOUNT_PATH" "$LV_SNAPSHOT_NAME" "$LV_SNAPSHOT_DISK" "$LV_DISK" ||
exit_error "Function hook_backup() failed"

echo "Unmounting snapshot..."
umount "$MOUNT_PATH" ||
exit_error "Failed to umount snapshot"
rmdir "$MOUNT_PATH"

echo "Removing snapshot ..."
lvremove -f "$VG_NAME/$LV_SNAPSHOT_NAME" ||
exit_error "Failed to remove snapshot"
